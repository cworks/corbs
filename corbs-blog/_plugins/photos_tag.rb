...
module Jekyll
  ...
  class PhotoTag < Liquid::Tag
    ...
    def render(context)
      ...
      if @filename
        "<a href=\"#{p.path_for(@filename)}\" title=\"#{@title}\"><img class=\"thumbnail\" src=\"#{p.thumb_for(@filename,@thumb)}\" alt=\"#{@title}\" /></a>"
      else
    ...
    def render(context)
      ...
      lines.each do |line|
        if /(?<filename>[^\[\]:]+)(?:\[(?<thumb>\S*)\])?(?::(?<title>.*))?/ =~ line
          ...
          list << "<img class=\"thumbnail\" src=\"#{p.thumb_for(filename,thumb)}\" alt=\"#{title.strip}\" /></a></li>"
        end
      end
      "<ul class=\"gallery\">\n#{list}\n</ul>"
    end
